...
contract Voting {
    ...
    function addVoteSession(
        ...
        if (bytes(_name).length == 0) revert NameVoteSessionCantBeEmpty();
    ...
    function endVoteSession(uint256 _voteSessionId) public {
        ...
        if (voteSession.status == StatusVoteSession.Ended) revert VoteSessionAlreadyEnded();
        if (voteSession.tempNumberVotes < voteSession.minNumberVotes) {
            ...
            for (uint256 i = 0; i < countChoices; i++) {
                if (voteSession.choices[i].countVotes > maxCountVotes) {
                    ...
                    winnerVoteSession = voteSession.choices[i].name;
                }
            }
            voteSession.winner = winnerVoteSession;
        }

        emit VoteSessionEnded(_voteSessionId, voteSession.tempNumberVotes, voteSession.status, voteSession.winner);
    }
}
